...
const TimeTracking: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 bg-white rounded-lg shadow-lg space-y-6">
      <h1 className="text-3xl font-semibold text-center text-gray-800 mb-6">Employee Time Tracker</h1>

      {/* Current Time */}
      <div className="text-center">
        <h2 className="text-xl font-medium text-gray-600">Current Time</h2>
        <p className="text-4xl font-bold text-gray-800">{currentTime}</p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex justify-between">
          <span className="font-medium text-gray-700">Lunch Out Time:</span>
          <span>{lunchOutTime ?? 'Not recorded'}</span>
        </div>
        <div className="flex justify-between">
          <span className="font-medium text-gray-700">Lunch In Time:</span>
      ...
      <div className="flex flex-wrap gap-4 justify-center">
        ...
        <button
          ...
          className={`px-6 py-2 w-40 rounded-md text-white focus:outline-none transition ${
            isLunchOut ? 'bg-gray-500 cursor-not-allowed' : 'bg-blue-500 hover:bg-blue-600'
          }`}
          disabled={isLunchOut}
        >
          Lunch Out
        ...
        <button
          ...
          className={`px-6 py-2 w-40 rounded-md text-white focus:outline-none transition ${
            isLunchIn ? 'bg-gray-500 cursor-not-allowed' : 'bg-blue-500 hover:bg-blue-600'
          }`}
          disabled={isLunchIn}
        >
          Lunch In
        ...
        <button
          ...
          className={`px-6 py-2 w-40 rounded-md text-white focus:outline-none transition ${
            isCheckedOut ? 'bg-gray-500 cursor-not-allowed' : 'bg-blue-500 hover:bg-blue-600'
          }`}
...
export default withAuth(TimeTracking);